...
// IMPORTANT: Ensure this path to your apiService.ts file is correct.
import {
  fetchOptions,
  addOption,
  saveTrade,
...
// IMPORTANT: Ensure this path to your shared types file is correct.
    ...
    <div className={Styles.overlay} onClick={onClose}>
      <div className={Styles.newTradePopup} onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className={Styles.form}>
          {activeTab === 'general' && (
            <>
              <div className={Styles.section}><h3>Trade Details</h3><div className={Styles.formGrid}>
                ...
                <div className={Styles.formGroup}><label>Direction <span className={Styles.required}>*</span></label><div className={Styles.directionContainer}><button type="button" className={`${Styles.directionBtn} ${formData.direction === 'Long' ? Styles.active : ''}`} onClick={() => handleUpdateField('direction', 'Long')}><IoMdArrowDropup className={Styles.directionIcon} /> Long</button><button type="button" className={`${Styles.directionBtn} ${formData.direction === 'Short' ? Styles.active : ''}`} onClick={() => handleUpdateField('direction', 'Short')}><IoMdArrowDropdown className={Styles.directionIcon} /> Short</button></div></div>
              ...
              <div className={Styles.section}><h3>Strategy & Analysis</h3>
                ...
                <div className={Styles.formGroup}><label>Trade Analysis <span className={Styles.required}>*</span></label><textarea value={formData.trade_analysis} onChange={(e) => handleUpdateField('trade_analysis', e.target.value)} rows={3} required /></div>
                <div className={Styles.formGroup}>
                  <label>Rules Followed <span className={Styles.required}>*</span></label>
                  <div className={Styles.multiSelectContainer}>
                    {rulesOptions.map(o => <label key={o._id} className={Styles.checkboxLabel}><input type="checkbox" checked={formData.rules_followed.includes(o._id)} onChange={() => handleMultiSelect(o._id)} />{o.name}</label>)}
                  </div>
                  <div className={Styles.addCustomContainer}>
              ...
              <div className={Styles.section}><h3>Performance</h3><div className={Styles.formGrid}><div className={Styles.formGroup}><label>P&L Amount</label><div className={Styles.calculatedField}>₹{formData.pnl_amount.toFixed(2)}</div></div><div className={Styles.formGroup}><label>P&L Percentage</label><div className={Styles.calculatedField}>{formData.pnl_percentage.toFixed(2)}%</div></div><div className={Styles.formGroup}><label>Holding Period (Minutes)</label><input type="number" value={formData.holding_period_minutes ?? ''} onChange={(e) => handleNumberInputChange('holding_period_minutes', e.target.value)} /></div></div></div>
              <div className={Styles.section}><h3>Tags</h3><div className={Styles.tagInputContainer}><input type="text" value={newTag} onChange={(e) => setNewTag(e.target.value)} placeholder="Add tag..." onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addTag())} /><button type="button" onClick={addTag}>Add</button></div><div className={Styles.tagsContainer}>{formData.tags.map(tag => <span key={tag} className={Styles.tag}>{tag}<button type="button" onClick={() => removeTag(tag)}>×</button></span>)}</div></div>
            </>
          )}
          {activeTab === 'psychology' && (
            <div className={Styles.section}><h3>Psychology</h3><div className={Styles.formGrid}>
                <div className={Styles.formGroup}><label>Entry Confidence: {formData.psychology.entry_confidence_level}/10 <span className={Styles.required}>*</span></label><input type="range" min="1" max="10" value={formData.psychology.entry_confidence_level} onChange={(e) => handlePsychologyChange({ entry_confidence_level: Number(e.target.value) })} required/></div>
                <div className={Styles.formGroup}><label>Satisfaction Rating: {formData.psychology.satisfaction_rating}/10 <span className={Styles.required}>*</span></label><input type="range" min="1" max="10" value={formData.psychology.satisfaction_rating} onChange={(e) => handlePsychologyChange({ satisfaction_rating: Number(e.target.value) })} required/></div>
                <div className={Styles.formGroup}><label>Emotional State <span className={Styles.required}>*</span></label><select value={formData.psychology.emotional_state} onChange={(e) => handlePsychologyChange({ emotional_state: e.target.value })} required><option value="">Select State</option>{emotionalStates.map(o => <option key={o._id} value={o._id}>{o.name}</option>)}</select></div>
              </div>
              <div className={Styles.formGroup}><label>Mistakes Made</label><div className={Styles.tagInputContainer}><input type="text" value={newMistake} onChange={(e) => setNewMistake(e.target.value)} placeholder="Add mistake..." onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addMistake())} /><button type="button" onClick={addMistake}>Add</button></div><div className={Styles.tagsContainer}>{formData.psychology.mistakes_made.map(mistake => <span key={mistake} className={Styles.tag}>{mistake}<button type="button" onClick={() => removeMistake(mistake)}>×</button></span>)}</div></div>
              <div className={Styles.formGroup}><label>Lessons Learned <span className={Styles.required}>*</span></label><textarea value={formData.psychology.lessons_learned} onChange={(e) => handlePsychologyChange({ lessons_learned: e.target.value })} rows={3} required/></div>
            </div>
          )}
          <div className={Styles.formActions}><button type="button" onClick={onClose} className={Styles.cancelBtn}>Cancel</button><button type="submit" className={Styles.submitBtn}>Save Trade</button></div>
        </form>
      </div>
    </div>
  );
